...
class StreamBuffer {
  ...
  public:
    ...
    std::span<std::byte> alloc(){
        data.insert(data.end(),layout.stride(),{})
    }
...
class Pipeline {
  ...
  public:
    ...
    void Execute() {
        ...
        StreamBuffer buffer0(geometry_layout);

        // 遍历顶点ID
        for (int i = 0; i < vertex_count; i++) {
            auto item = data.fetch(i);
            auto buffer = buffer0.data_view(i);
        ...
        transform_layout.add(Sematic::SV_Position); // NDC Position(/w)
        StreamBuffer transform_buffer(transform_layout);

        for(int i=0;i<vertex_count;i++)
        {
            auto buffer = buffer0.data_view(i);
            // TODO: buffer[i]/w -> transform_buffer
        ...
        raster_layout.add(Semetic::SV_Position);    // World Position
        raster_layout.add(Semetic::SV_ID); // Triangle ID
        raster_layout.add(Semetic::SV_Lerp_Param);  
